...
- **MCP 工具**
  - `pull`：拉取记忆，首行为库元信息（`── 库 2026-09-10 起 · 共 27 条 · 本次注入 7 条 ──`），其后置顶记忆在前、短期记忆在后（注入格式：`[a3f 2026-09-07] 内容`，日期为**创建日**，被编辑过则显示 `创建日→更新日`；id 为随机字母数字，长度由 `ID_LENGTH` 控制，默认 3）。默认取最近 5 条，可传 `extend`（0-20）在此基础上往后追加，像翻页；置顶恒定全部返回
  - `write`：创建一条新记忆，自动附加时间戳
  - `edit`：按 id（pull 返回行首的字母编号）修改记忆，自动刷新时间戳
  - `pin`：置顶/解除置顶某条记忆（unpin=true 解除；置顶上限 15 条）
  - `search`：单关键词搜索记忆（子串匹配，默认最多 10 条，满额时提示可能被截断；可限定 since/until 日期范围，按写入日过滤）。keyword 留空时按时间列出该范围内的记忆，此时必须带 since 或 until
