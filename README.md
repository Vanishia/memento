...
- **MCP 工具**
  - `pull`：拉取记忆，首行为库元信息（`── 库 2026-09-10 起 · 共 27 条 · 本次注入 7 条 ──`），其后置顶记忆在前、短期记忆在后（注入格式：`[a3f 2026-09-07] 内容`，id 为随机字母数字，长度由 `ID_LENGTH` 控制，默认 3）。短期条数可传 `limit`（默认 5、上限 50），置顶恒定全部返回
